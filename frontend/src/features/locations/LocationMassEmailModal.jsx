...
const LocationMassEmailModal = ({
  ...
  const templates = [
    ...
    },
    {
      ...
      content: `<div style="font-family: Arial; padding: 20px;"><h2 style="color:#005bbb;">Location Announcement</h2><p>Dear <strong>\${Contact.Name}</strong>,</p><p>This is to inform your location <strong>\${Location.Name}</strong> has been updated.</p><p>Please review the changes at your earliest convenience.</p><p>Thank you!</p></div>`,
    },
  ...
  const sendMassEmail = () => {
    ...
    onSend({
      from: selectedFrom,
      locations: selectedLocationEmails,
    ...
    <>
      <Drawer open={isOpen} onOpenChange={handleClose} direction="right">
        <DrawerContent className="max-w-4xl max-h-[100vh] flex flex-col">
          <DrawerHeader>
            <DrawerTitle>Mass Email Locations</DrawerTitle>
            <DrawerDescription>Send emails to selected locations.</DrawerDescription>
          </DrawerHeader>
          <div className="flex-1 overflow-y-auto space-y-4 p-4">
            <div>
              <Label>To</Label>
              <div className="mt-1 text-sm">
                {selectedLocationEmails?.length ? (
                  selectedLocationEmails.map((location, idx) => (
                    <span key={location.ROWID}>
                      {location.locationName}
                      {idx < selectedLocationEmails.length - 1 ? ", " : ""}
                    </span>
                  ))
                ) : (
...
export default LocationMassEmailModal;
