...
const MassUpdateEmailModal = ({ isOpen, fields, selectedIds, selectedEmailAdvisors, onClose, onSend }) => {
  ...
  const sendMassEmail = () => {
    ...
    onSend({
      from: selectedFrom,
      advisors: selectedEmailAdvisors,
      ...
      <Dialog open={isOpen} onOpenChange={handleClose}>
        <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              Send emails to selected advisors.
            </DialogDescription>
          </DialogHeader>
          <div className="flex-1 overflow-y-auto space-y-4">
            <div>
              ...
              <div className="mt-1 text-sm">
                {selectedEmailAdvisors?.length ? (
                  selectedEmailAdvisors.map((advisor, idx) => (
                    <span key={advisor.ROWID}>
                      {`${advisor.firstName ?? ""} ${advisor.lastName ?? ""}`.trim()}{idx < selectedEmailAdvisors.length - 1 ? ', ' : ''}
                    </span>
                  ))
                ) : (
