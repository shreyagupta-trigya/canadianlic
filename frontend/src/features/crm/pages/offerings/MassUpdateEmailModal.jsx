    ...
    <>
      <Drawer open={isOpen} onOpenChange={handleClose} direction="right">
        <DrawerContent className="max-w-4xl max-h-[100vh] flex flex-col">
          <DrawerHeader>
            <DrawerTitle>Mass Email</DrawerTitle>
            <DrawerDescription>
              Send emails to selected offerings.
            </DrawerDescription>
          </DrawerHeader>
            ...
            <div>
              ...
              <div className="mt-1 text-sm">
                {selectedEmailLeads?.length ? (
                  selectedEmailLeads.map((offering, idx) => (
                    <span key={offering.ROWID}>
                      {offering.offeringName}{idx < selectedEmailLeads.length - 1 ? ', ' : ''}
                    </span>
                  ))
                ) : (
