...
export const validateAdvisorListData = async (payload = {}) => {
  try {
    ...
    const result = jsonData.find((item) => item.moduleName === "advisor");
    ...
    const advisorList = await axios.post(
      `${putUrl}advisorFunction/api/v1/getalladvisors`,payload,config
    );

    console.log("advisorl List",advisorList);
    ...
    return [showAndHideState, advisorList.data.adv ];
  } catch (error) {
