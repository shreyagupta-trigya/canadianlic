...
export const validatePartnerListData = async (payload={}) => {
  try {
    ...
    const result = jsonData.find((item) => item.moduleName === "partner");
    ...
    const response = await axios.post(`${putUrl}insurancePartner/api/v1/getinsurancepartner`,payload, config);




    console.log("InsuranceOptionResponse", response);
    // Determine user role
