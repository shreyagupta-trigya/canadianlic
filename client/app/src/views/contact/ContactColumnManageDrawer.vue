...
<script>
export default {
  name: 'ContactColumnManageDrawer',
  ...
  methods: {
    ...
    // Only update localColumns, do not emit toggle-column (ContactList.vue expects update-columns on Save only)
    updateVisibleList({index, label, checked}) {
      if (this.localColumns[index]) {
        this.localColumns[index].visible = checked;
      }
      // No emit here; parent only updates on Save
    },
