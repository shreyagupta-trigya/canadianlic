...
async function remoteDataParser(data) {
    return {
        ...
        // Convert BigInt to string before sending it (if needed)
owner: data?.owner ? data.owner.toString() : null,

       // exchangeRate: data?.exchangeRate ?? '',
       exchangeRate: data?.exchangeRate !== undefined && data?.exchangeRate !== null && !isNaN(data?.exchangeRate) ? Number(data?.exchangeRate) : 0,
        ...
        leadId: data?.leadId ? data.leadId.toString() : null,
        contactId: data?.contactId ? data.contactId.toString() : null,
        // contactId: data?.contactId ?? null,
        sessionId: data?.sessionId ?? '',
        digest: data?.digest ?? '',
        onDemandSession: data?.onDemandSession ?? false,
