...
const queries = require("../../SQL/refferalQuery");
const {
  insertData,
  insertSubformData,
  dateTimeFormat,
...
exports.testConnection = async (req, res) => {
  try {
    ...
    res.status(409).json({ success: true, message: "I'm Not live!" });
  }
};
  ...
  try {
    ...
    const { updateArray: updateSubform = [], insertArray: insertSubform = [] } =
      (await processSubform(
        refferalDetails?.numberOfReferralScoreGridData ?? "",
        referralId,
        "referralScoreboard"
      )) || {};
...
exports.countRefferal = async (req, res) => {
  const adminApp = catalyst.initialize(req, { scope: "admin" });
  let countQuery = "SELECT COUNT(ROWID) FROM referralData";
  try {
      ...
      const total = data?.[0]?.referralData?.total;
      res.status(200).json({
          success: true,
          message: "Referral Data Count Fetched Successfully",
          count: total
      });
  } catch (error) {
      res.status(409).json({
          success: false,
          message: "Referral Data Count Fetch Issue",
          error: error
      });
  }
}
...
async function getReferralLeadDBParser(result) {
  return result.map((item) => {
    return {
      ...
      leadName:
        `${item.leads?.firstName ?? ""} ${item.leads?.lastName ?? ""}` ?? null,
      leadId: item.referralData?.leadId ?? "",
    };
  });
}
