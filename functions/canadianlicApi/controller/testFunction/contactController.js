...
exports.createContact = async (req, res) => {
  try {
    ...
    const contactDa = req.body;
    const contactData = contactDa?.data?.event_data[0];
    // console.log("contactDa==>", contactData);
    const conResp = await parseContactsData(catalystApp,contactData);
    res
      .status(200)
      .json({
        success: true,
    ...
    res.status(404).json({ success: false, message: "Contact not created!" });
  }
};
