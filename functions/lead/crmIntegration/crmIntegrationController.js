...
// const CrmBaseUrl = process.env.CRMURL

// const {getLocationsCrmId} = require("../../canadianlicApi/Utils/crmLookupHelper");
...
exports.dataSyncZcrm = async (token,formData, crmId) => {
  ...
  try {
    ...
    const leadInfo = await leadParseData(formData?.LeadInformation,token);
...
async function leadParseData(formData,token){
  return {
    ...
    Remove_From_Campaign: formData?.removeFromCampaign??false,
    Email_is_Valid: formData?.emailIsValid??"",
    // CrmAdvisorId: await getLocationsCrmId(formData?.locationName,token),
    // Assigned_to_Advisor: locationId,
...
async function parseFacebook(facebook) {
  return{
    ...
    //Cost_Per_Lead_CPL:facebook?.costPerLead??"",//currency in crm
...
async function getLocationsCrmId(catalystId, accessToken) {
  const  queryArr = [
      `select Catalyst_Id as Accounts_Client from Accounts where ((Catalyst_Id = '${catalystId}'))  limit 1`,
      `select Catalyst_Id as Accounts_Client from Accounts where ((Catalyst_Id = '${catalystId}'))  limit 1`,
  ]
