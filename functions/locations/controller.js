const catalyst = require("zcatalyst-sdk-node");
const { location, sample } = require("./exports");
const searchQueryBuilder = require("./searchQueryBuilder");
const { createObjectCsvStringifier } = require("csv-writer");
const {
  insertDataIntoTable,
  insertMultipleRowsIntoTable,
  decryptData,
} = require("./utils");
const {
  dataSyncZcrm,
  generateToken,
} = require("./crmIntegration/crmIntegrationController");

const {getSequence,updateSequence} = require("./sequenceUtils");

exports.createLocation = async (req, res) => {
  const formData = req.body;
  if (formData.parentLocation === "") {
    delete formData.parentLocation;
  }
  try {
    const catalystApp = catalyst.initialize(req, { scope: "admin" });
    const locationDataWithValues = await parseLocation(formData);
    const locationData = Object.fromEntries(
      Object.entries(locationDataWithValues).filter(([key, value]) => value !== "" && value !== null)
    );
      //  // SEQUENCE CODE
       const sequenceResp = await getSequence(req,"locations");
       let sequence =  sequenceResp?.data
       let sourceId = `${sequence?.prefix}-${sequence?.sequence.padStart(5,'0')}`;
       locationData.sourceId = sourceId;
 
    const locationResult = await insertDataIntoTable(
      catalystApp,
      "locations",
      locationData
    );
    const id = locationResult?.ROWID;
    await updateSequence(req,sequence?.rowId,parseInt(sequence.sequence) + 1);

    // // ************* CRM FUNCTION ***************
    // const token = await generateToken();
    // // console.log(token);
    // const crmId = await dataSyncZcrm(token, { ...formData, ROWID: id });
    // // console.log("crmId:===> ", crmId);
    // await catalystApp
    //   .datastore()
    //   .table("locations")
    //   .updateRow({ sourceId: crmId, source: "catalyst", ROWID: id });

    let RollingResult;
    if (
      formData?.Rolling?.RollingData &&
      formData?.Rolling?.RollingData?.length > 0
    ) {
      RollingResult = await insertMultipleRowsIntoTable(
        "locationRolling",
        formData?.Rolling?.RollingData,
        req,
        locationResult
      );
    }
    // console.log("RollingResult", RollingResult);

    let AnnualPerformanceResult;
    if (
      formData?.AnnualPerformance?.AnnualPerformance &&
      formData?.AnnualPerformance?.AnnualPerformance?.length > 0
    ) {
      AnnualPerformanceResult = await insertMultipleRowsIntoTable(
        "locationAnnualPerformance",
        formData?.AnnualPerformance?.AnnualPerformance,
        req,
        locationResult
      );
    }
    console.log("AnnualPerformanceResult", AnnualPerformanceResult);
    const responseData = {
      location: locationResult,
    };
    //  Node.del("contactList")
    res.status(201).json({
      success: true,
      message: "Location successfully",
      rowId: locationResult.ROWID,
      // crmId,
    });
  } catch (error) {
    console.error("Error creating contact:", error);
    res.status(409).json({
      success: false,
      message: "Failed to create Lead",
      error: error,
    });
  }
};

exports.getLocation = async(req, res) => {
  try {                                         
    const { search } = req.body;
    const rowId = req.params.id;
    const page = parseInt(req.body.page, 10) || 1;
    const limit = parseInt(req.body.limit, 10) || 10;
    const offset = (page - 1) * limit;
    const fieldMapping = { locationName:"locationName", locationOwner:["userData.firstName","userData.lastName"]};                                              // closes fieldMapping

    let searchConditions = searchQueryBuilder(search, fieldMapping);
  
    if (rowId) {                                    
      const rowIdCondition = `ROWID = '${rowId}'`;
      searchConditions = searchConditions ? `${searchConditions} AND ${rowIdCondition}` : rowIdCondition;
    }                                               

    const whereClause = searchConditions ? `WHERE (${searchConditions})` : '';

    let getLocationQuery = `SELECT locations.*,userData.firstName,userData.lastName 
      FROM locations 
      LEFT JOIN userData ON userData.ROWID=locations.locationOwner 
      %SEARCH_CONDITION%  
      ORDER BY CREATEDTIME DESC 
      LIMIT %LIMIT% OFFSET %OFFSET%`;

    let query = getLocationQuery
      .replace('%SEARCH_CONDITION%', whereClause)
      .replace('%LIMIT%', limit)
      .replace('%OFFSET%', offset);
      const locationData = await catalyst.initialize(req, { scope: "admin" }).zcql().executeZCQLQuery(query);
      return res.status(200).json(locationData);
    } catch (error) {
      return res.status(500).json({
        success: false,
        message: "Error fetching location data",
        error: error.message,
      })
    }
}

exports.getSingleLocation = (req, res) => {
  console.log(req.params.id);

  const query = `SELECT * from locations WHERE ROWID=${req.params.id}`;

  catalyst
    .initialize(req, { scope: "admin" })
    .zcql()
    .executeZCQLQuery(query)
    .then((data) => {
      res.status(201).json(data);
    })
    .catch((error) => {
      res.status(403).json({
        succuss: false,
        message: "Issue pulling location data",
        output: error,
      });
    });
};
exports.updateLocation = async (req, res) => {
  // console.log(req.params.id);

  req.body.ROWID = req.params.id;
  const formData = req.body;
  // const locationUpdate = {
  //   locationOwner: formData.locationOwner ? formData.locationOwner : null,
  //   locationName: formData.locationName ? formData.locationName : null,
  //   email: formData.email ? formData.email : null,
  //   phone: formData.phone ? formData.phone : null,
  //   fax: formData.fax ? formData.fax : null,
  //   website: formData.website ? formData.website : null,
  //   rating: formData.rating ? formData.rating : null,
  //   description: formData.description ? formData.description : null,
  //   currency: formData.currency ? formData.currency : null,
  //   parentLocation: formData.parentLocation ? formData.parentLocation : null,
  //   employees: formData.employees ? formData.employees : null,
  //   discountFactor: formData.discountFactor ? formData.discountFactor : null,
  //   exchangeRate: formData.exchangeRate ? formData.exchangeRate : null,
  //   // phoneBurnerFollowUpDate: formData.phoneBurnerFollowUpDate ? formData.phoneBurnerFollowUpDate : null,
  //   // phoneBurnerLastCallOutcome: formData.phoneBurnerLastCallOutcome ? formData.phoneBurnerLastCallOutcome : null,
  //   // phoneBurnerLastCallTime: formData.phoneBurnerLastCallTime ? formData.phoneBurnerLastCallTime : null,
  //   // month: formData.month ? formData.month : null,
  //   // year: formData.year ? formData.year : null,
  //   // monthlyRevenue: formData.monthlyRevenue ? formData.monthlyRevenue : null,
  //   // numberOfAdvisors: formData.numberOfAdvisors ? formData.numberOfAdvisors : null,
  //   // numberOfClients: formData.numberOfClients ? formData.numberOfClients : null,
  //   // advisorMonthlyYield: formData.advisorMonthlyYield ? formData.advisorMonthlyYield : null,
  //   // clientMonthlyYield: formData.clientMonthlyYield ? formData.clientMonthlyYield : null,
  //   // averageMonthlyRevenue: formData.averageMonthlyRevenue ? formData.averageMonthlyRevenue : null,
  //   // advisorAverageMonthlyYield: formData.advisorAverageMonthlyYield ? formData.advisorAverageMonthlyYield : null,
  //   // clientAverageMonthlyYield: formData.clientAverageMonthlyYield ? formData.clientAverageMonthlyYield : null,
  //   // totalRollingRevenue: formData.totalRollingRevenue ? formData.totalRollingRevenue : null,
  //   // numberOfAdvisorsAtYearEnd: formData.numberOfAdvisorsAtYearEnd ? formData.numberOfAdvisorsAtYearEnd : null,
  //   // clientsAtYearEnd: formData.clientsAtYearEnd ? formData.clientsAtYearEnd : null,
  //   // annualRevenue: formData.annualRevenue ? formData.annualRevenue : null,
  //   // advisorAnnualRevenueYield: formData.advisorAnnualRevenueYield ? formData.advisorAnnualRevenueYield : null,
  //   // clientAnnualRevenueYield: formData.clientAnnualRevenueYield ? formData.clientAnnualRevenueYield : null,
  //   // advisorAverageAnnualYield: formData.advisorAverageAnnualYield ? formData.advisorAverageAnnualYield : null,
  //   // clientAverageAnnualYield: formData.clientAverageAnnualYield ? formData.clientAverageAnnualYield : null,
  //   street: formData.street ? formData.street : null,
  //   city: formData.city ? formData.city : null,
  //   state: formData.state ? formData.state : null,
  //   postalCode: formData.postalCode ? formData.postalCode : null,
  //   country: formData.country ? formData.country : null,
  //   shipmentStreet: formData.shipmentStreet ? formData.shipmentStreet : null,
  //   shipmentCity: formData.shipmentCity ? formData.shipmentCity : null,
  //   shipmentState: formData.shipmentState ? formData.shipmentState : null,
  //   shipmentPostalCode: formData.shipmentPostalCode
  //     ? formData.shipmentPostalCode
  //     : null,
  //   shipmentCountry: formData.shipmentCountry ? formData.shipmentCountry : null,
  //   ROWID: req.params.id,
  // };
  const locationUpdateWithValues = await parseLocation(formData);
  const locationUpdate = Object.fromEntries(
    Object.entries(locationUpdateWithValues).filter(([key, value]) => value !== "" && value !== null)
  );
  const catalystApp = catalyst.initialize(req, { scope: "admin" });
  catalystApp
    .datastore()
    .table("locations")
    .updateRow({...locationUpdate, ROWID:req.params.id})
    .then((row) =>
      res
        .status(201)
        .json({
          success: true,
          message: "Updated location successfully",
          output: row,
        })
    )
    .catch((error) =>
      res
        .status(409)
        .json({ success: false, message: "Updated Faild", output: error })
    );
};
exports.deleteLocation = async (req, res) => {
  console.log(req.params.id);
  catalyst
    .initialize(req, { scope: "admin" })
    .datastore()
    .table("locations")
    .deleteRow(req.params.id)
    .then((data) => {
      res.status(201).json({
        success: true,
        message: "location deleted successfully",
        data,
      });
    })
    .catch((error) => {
      console.log(error);
      res
        .status(406)
        .json({
          succuss: false,
          message: "Issue with deleting location",
          output: error,
        });
    });
};

exports.getSingleLocationData = async (req, res) => {
  console.log(req.params.id);
  try {
    const query = `SELECT * from locations WHERE ROWID=${req.params.id}`;
    const location = await catalyst
      .initialize(req, { scope: "admin" })
      .zcql()
      .executeZCQLQuery(query);

    const userQuery = `SELECT * from userData WHERE ROWID=${location[0].locations.locationOwner}`;
    const locationOwner = await catalyst
      .initialize(req, { scope: "admin" })
      .zcql()
      .executeZCQLQuery(userQuery);

    const policyQuery = `SELECT * from policies WHERE location=${req.params.id}`;
    const policies = await catalyst
      .initialize(req, { scope: "admin" })
      .zcql()
      .executeZCQLQuery(policyQuery);

    const contactQuery = `SELECT * from contacts WHERE location=${req.params.id}`;
    const contacts = await catalyst
      .initialize(req, { scope: "admin" })
      .zcql()
      .executeZCQLQuery(contactQuery);

    const advisorQuery = `SELECT * from advisors WHERE location=${req.params.id}`;
    const advisors = await catalyst
      .initialize(req, { scope: "admin" })
      .zcql()
      .executeZCQLQuery(advisorQuery);
    res
      .status(200)
      .json({
        success: true,
        message: "location data",
        location,
        locationOwner,
        policies,
        contacts,
        advisors,
      });
  } catch (error) {
    console.log(error);
    res
      .status(500)
      .json({ success: false, message: "faild to get location data", error });
  }
};

exports.downloadFile = async (req, res) => {
  try {
    const csvContent = await downloadSampleFile();

    // Send the CSV file for download
    res.setHeader("Content-disposition", "attachment; filename=sample.csv");
    res.set("Content-Type", "text/csv");
    res.status(200).send(csvContent);
  } catch (error) {
    console.error("Error downloading file:", error);
  }
};

async function downloadSampleFile() {
  try {
    // console.log("This is insurance partner download file",partner)

    let headers = location;
    const csvStringifier = createObjectCsvStringifier({
      header: headers,
    });
    const data = sample;
    const csvContent =
      csvStringifier.getHeaderString() + csvStringifier.stringifyRecords(data);

    return csvContent;
  } catch (error) {
    console.error("Error downloading file:", error);
  }
}

// **********PARSE LOCATION DATA **********

async function parseLocation(formData) {
  return {
    locationOwner: formData?.locationOwner ?? null,
    locationName: formData?.locationName ?? "",
    email: formData?.email ?? "",
    phone: formData?.phone ?? null,
    fax: formData?.fax ?? "",
    website: formData?.website ?? "",
    rating: formData?.rating ?? "",
    description: formData?.description ?? "",
    currency: formData?.currency ?? "",
    // parentLocation: BigInt(formData?.parentLocation )?? null,
    parentLocation: formData?.parentLocation != null ? BigInt(formData.parentLocation) : null,

    employees: formData?.employees ?? "",
    discountFactor: formData?.discountFactor ?? "",
    exchangeRate: formData?.exchangeRate ?? "",
    // phoneBurnerFollowUpDate: formData?.phoneBurnerFollowUpDate ??null,
    street: formData?.street ?? "",
    city: formData?.city ?? "",
    state: formData?.state ?? "",
    postalCode: formData?.postalCode ?? "",
    country: formData?.country ?? "",
    shipmentStreet: formData?.shipmentStreet ?? "",
    shipmentCity: formData?.shipmentCity ?? "",
    shipmentState: formData?.shipmentState ?? "",
    shipmentPostalCode: formData?.shipmentPostalCode ?? "",
    shipmentCountry: formData?.shipmentCountry ?? "",
  };
}
