...
const queries = require("../../query/contact/query");

const { insertData, insertSubformData, dateTimeFormat, updateData, updateSubformData, } = require("../../utils/contact/util");
...
exports.updateContact = async function(req, res) {
  ...
  const leadinfoData = await parseleadInformationsData(leadinfo,facebook,contactId);
  const leadInformationsId = await updateData(catalystApp, "leadInformations", {...leadinfoData, ROWID:leadinfo.ROWID});

  const familyTreeData = await parseFamilyTreeData(familyTree,contactId);
  const familyTreeId = await insertData(catalystApp, "familyTree", {...familyTreeData, ROWID:familyTree.ROWID});
